...
PostCleaning::PostCleaning(QWidget *parent) : QWidget(parent)
{
    ...
    DLabel *pDLabel = new DLabel(tr("后期清理"));
    QFont font;
    ...
    connect(pDCommandLinkButton, &DCommandLinkButton::clicked, this, [=] {
        ...
        for (QString strSelectFile : strlistSelectedName) {
            ...
            pDLabeldeb->show();
            connect(pDCommandLinkButtonclear, &DCommandLinkButton::clicked, this, &PostCleaning::slotDpushBtnClearClicked);
    ...
    connect(pDpushBtn2, &DPushButton::clicked, this, &PostCleaning::slotDpushBtnNextClicked);
...
void PostCleaning::slotDpushBtnNextClicked()
{
    qDebug()<<"next";
    emit toChooseKermel();

}
void PostCleaning::slotDpushBtnClearClicked()
{
    qDebug()<<"clear";
    pDLabeldeb->close();
}
